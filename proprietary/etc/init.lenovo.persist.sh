...
#wuzb1 2016-10-26 KUNTAO-4850 allow chown rfs file used by tftp_server
...
# Lenovo wuzb1 2016-10-26 KUNTAO-4850 allow chown rfs file used by tftp_server
chown_modem_rfs_file() {
	rfs_uid=`id -u rfs`
	RFS_DIR=/persist/rfs
	if [ -e $RFS_DIR ]; then
		rfs_owner=`stat -c %u $RFS_DIR`
		if [ $rfs_owner != $rfs_uid ]; then
			/system/bin/chown -R rfs:rfs $RFS_DIR
		fi
	fi

	HLOS_DIR=/persist/hlos_rfs
	if [ -e $HLOS_DIR ]; then
		hlos_owner=`stat -c %u $HLOS_DIR`
		if [ $hlos_owner != $rfs_uid ]; then
			/system/bin/chown -R rfs:rfs_shared $HLOS_DIR
		fi
	fi
}
